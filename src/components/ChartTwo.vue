<template>
  <div
    class="col-span-12 rounded-sm border border-gray-200 bg-white p-5 shadow-md hover:shadow-2xl duration-300 xl:col-span-4">
    <div class="mb-4 justify-between gap-4 sm:flex">
      <div>
        <h4 class="text-xl font-bold text-gray-800">Dias da semana</h4>
      </div>
      <div>
        <div class="relative z-20 inline-block">
          <select @change="toggle()" name="selectChartTwo" id="selectChartTwo"
...
async function toggle() {
  const selected = document.getElementById("selectChartTwo").value;
  chartData.value = await StatisticService.getStats(JSON.stringify({
    id: 3, interval: selected,
  }));
}
</script>